...
class com.rockstargames.ui.tweenStar.TweenStarLite
{
    ...
    function TweenStarLite(_mytarget, _duration, _vars, _isTween)
    {
        ...
        target = _mytarget;
        vars = _vars;
        duration = _duration * com.rockstargames.ui.tweenStar.TweenStarLite.frameRate;
        startTime = vars.delay * -com.rockstargames.ui.tweenStar.TweenStarLite.frameRate || 0;
        ease = com.rockstargames.ui.tweenStar.TweenStarLite.linearEase;
        props = [];
        if (_isTween)
        {
    ...
    {
        ++startTime;
        var _loc2 = startTime / duration;
        _loc2 = Math.min(_loc2, 1);
        for (var _loc3 in props)
        {
            if (_loc2 > 0)
            {
                target[props[_loc3][0]] = this.ease(startTime, props[_loc3][1], props[_loc3][2] - props[_loc3][1], duration);
            } // end if
        } // end of for...in
        if (_loc2 == 1)
        {
