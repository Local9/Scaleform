...
// [Initial MovieClip Action of sprite 109]
#initclip 5
class com.rockstargames.gtav.levelDesign.INSTRUCTIONAL_BUTTONS extends com.rockstargames.ui.core.BaseScreenLayout
{
    ...
    function createItem(item)
    {
        ...
        for (var _loc2 = 0; _loc2 < item.length; ++_loc2)
        {
            ...
            _loc5._x = _loc5._x - (_loc6 + spaceBetweenSpaceAndButton) - PADDING / 2 - TEXT_BOX_PADDING;
        } // end of for
        item.reverse();
        return (_loc5);
    } // End of the function
    function createButtonIcon(buttonID, buttonParent)
    {
        ...
        {
            _loc3 = buttonParent.attachMovie("face_button_" + buttonID, "face_button_" + buttonID, buttonID, {_width: 20, _height: 20, _x: 2, _y: 2});
        }
        else
        {
            _loc3 = buttonParent.attachMovie("button_" + buttonID, "button_" + buttonID, buttonID, {_width: 20, _height: 20, _x: 2, _y: 2});
        } // end else if
        if (buttonID == 44)
        {
    ...
    function SET_BACKGROUND()
    {
        ...
        _loc4._visible = this.requiresBackground();
        this.initScreenLayout();
    } // End of the function
    function generateTextField(textFieldName, textString, parentMovieClip, buttonID)
    {
        ...
        {
            if (textString.toUpperCase().split("PLAYSTATION").length > 1)
            {
                if (textString.toUpperCase().split("STORE").length > 1)
                {
                    textString = "PlayStation" + String.fromCharCode(174) + "Store";
                } // end if
            }
            else if (buttonID < 48 || buttonID > 50)
            {
                textString = this.toTitleCase(textString);
            } // end else if
    ...
    function OVERRIDE_RESPAWN_TEXT(id, txt)
    {
        ...
        _loc2.text = this.toTitleCase(txt);
        _loc2.setTextFormat(instructionalTextFormat);
    } // End of the function
    ...
    function requiresBackground()
    {
        if (dataArray.length <= 1)
        {
            if (dataArray[0][1] == "")
            {
                if (dataArray[0][0] == 44 || dataArray[0][0] == 48 || dataArray[0][0] == 49 || dataArray[0][0] == 50)
                {
                    return (false);
                } // end if
            } // end if
    ...
    {
        this.CLEAR_ALL();
        this.SET_CLEAR_SPACE(0);
        this.SET_DATA_SLOT(0, 30, "LOADING");
        this.SET_DATA_SLOT(1, 31, "BACK");
        this.SET_DATA_SLOT(2, 32, "no button");
        this.SET_DATA_SLOT(3, 33, "still no button");
