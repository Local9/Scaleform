...
class com.rockstargames.gtav.cellphone.badger.CELLPHONE_BADGER extends MovieClip
{
    ...
    function CELLPHONE_BADGER()
    {
        ...
        style.push(new Array([5, 141, 243], [76, 143, 192], whiteRGB, offWhiteRGB, [210, 210, 210]));
        style.push(new Array([32, 132, 39], [57, 160, 51], whiteRGB, offWhiteRGB, [210, 210, 210]));
        style.push(new Array([243, 5, 5], [223, 50, 50], whiteRGB, offWhiteRGB, [210, 210, 210]));
        ...
        style.push(new Array([86, 12, 123], [131, 78, 158], whiteRGB, offWhiteRGB, [210, 210, 210]));
    ...
    {
        CONTENT.infoBar.sleepModeIcon._visible = isSleepModeActive;
    } // End of the function
    ...
    function updateInfoBar(currentClip)
    {
        ...
        if (currentClip.isLandscape)
        {
            ...
            infoBar._x = 256;
            infoBar._y = 0;
            infoBar.batteryIcon._visible = false;
            infoBar.timeBar.gotoAndStop("SMALL");
            infoBar.infobarBackground.gotoAndStop("LANDSCAPE");
            infoBar.timeBar._x = 30;
            infoBar.timeBar._y = 0;
            infoBar.dayBar._visible = false;
            infoBar.signalStrengthIcon._x = 316;
            infoBar.badgerIcon._x = 286;
            infoBar.sleepModeIcon._x = 262;
            infoBar.headerText._x = 0;
        ...
        else
        {
            ...
            infoBar._x = 0;
            infoBar._y = 0;
            infoBar.signalStrengthIcon._x = 235;
            infoBar.badgerIcon._x = 207;
            infoBar.sleepModeIcon._x = 184;
            infoBar.headerText._width = 256;
            infoBar.headerBackground._width = 256;
            if (currentClip._name == "HOMEMENU")
            {
                infoBar.infobarBackground.gotoAndStop("DEFAULT");
                infoBar.batteryIcon._visible = true;
                infoBar.timeBar.gotoAndStop("BIG");
                infoBar.timeBar._x = 128;
                infoBar.timeBar._y = 1;
                infoBar.dayBar._visible = true;
                infoBar.headerText._x = 256;
                infoBar.headerBackground._x = 256;
            }
            else
            {
                infoBar.infobarBackground.gotoAndStop("PORTRAIT");
                infoBar.batteryIcon._visible = false;
                infoBar.timeBar.gotoAndStop("SMALL");
                infoBar.timeBar._x = 30;
                infoBar.timeBar._y = 1;
    ...
    function COLOUR_BACKGROUND()
    {
        ...
        {
            if (CONTENT.phoneBackground.bgMC != undefined)
            {
                if (currentStyle[1][0] == currentStyle[1][1] && currentStyle[1][1] == currentStyle[1][2])
                {
                    com.rockstargames.ui.utils.Colour.Colourise(CONTENT.phoneBackground.bgMC, greyRGB[0], greyRGB[1], greyRGB[2], 100);
                }
                else
                {
                    com.rockstargames.ui.utils.Colour.Colourise(CONTENT.phoneBackground.bgMC, currentStyle[1][0], currentStyle[1][1], currentStyle[1][2], 100);
                } // end if
            } // end if
        } // end else if
    } // End of the function
